  ...
  <el-tabs type="border-card" :stretch="true">
    <el-tab-pane label="表单项属性">
      ...
      <div
        class="config-item"
        v-if="
          itemConfig.renderConfig &&
            itemConfig.renderConfig.defaultValue !== undefined &&
            canSetDefalutValueList.indexOf(itemConfig.type) > -1
        "
      >
        <label>默认值: </label>
        <el-input
        ...
        ></el-input>
        <el-rate
          v-if="itemConfig.type == 'rate'"
          style="display:inline-block;vertical-align: middle;"
          :max="data.renderConfig.max"
      ...
      <div
        ...
        v-if="
          itemConfig.renderConfig &&
            itemConfig.renderConfig.options !== undefined
        "
      >
        <label>选项: </label>
        <template
          v-if="
            itemConfig.type == 'radio' ||
              (itemConfig.type == 'select' && !itemConfig.renderConfig.multiple)
          "
        >
          <el-radio-group v-model="itemConfig.renderConfig.defaultValue">
            ...
            >
              <li
                v-for="(item, index) in itemConfig.renderConfig.options"
                :key="index"
              >
      ...
      <div
        ...
        v-if="
          itemConfig.renderConfig &&
            itemConfig.renderConfig.disabled !== undefined
        "
      >
        <label>操作属性: </label>
        <el-checkbox v-model="itemConfig.renderConfig.disabled"
          >禁用</el-checkbox
        >
      </div>
      <!-- required -->
      <div
        ...
        v-if="
          itemConfig.renderConfig &&
            itemConfig.renderConfig.required !== undefined
        "
      >
        <label>校验: </label>
        <el-checkbox v-model="itemConfig.renderConfig.required"
          >必填</el-checkbox
        >
      </div>
    </el-tab-pane>
    <el-tab-pane label="表单属性">
      ...
      <div class="config-item">
        <label>标签宽度: </label>
        <el-input-number
          v-model.number="formConfig.labelWidth"
          :step="10"
          :min="0"
          :max="200"
        ></el-input-number>
    ...
    validateRequired(val) {
      if (val) {
        this.validator.required = {
          required: true,
          message: `${this.itemConfig.label}必须填写`
        };
      } else {
