<template>
  <div class="model-container">
    <div class="form-empty" v-if="widgetConfig.formItems.length === 0">
      从左侧拖拽来添加表单项
    </div>
    <el-form
      ref="iForm"
    ...
    >
      ...
      >
        <el-form-item
          ...
          :label-width="
            item.renderConfig.labelWidth
              ? item.renderConfig.labelWidth + 'px'
              : undefined
          "
          :columns="
            item.renderConfig.columns || widgetConfig.formConfig.columns
          "
        ...
        >
          <template slot="label">
            ...
            >
              ...
              <div
                slot="content"
                v-html="
                  item.renderConfig.labelTips.content ||
                    item.renderConfig.labelTips
                "
              ></div>
            </el-tooltip>
          </template>
          ...
          <div
            ...
            v-if="selectWidget._uniqueKey == item._uniqueKey"
          >
    ...
    <el-dialog title="表单预览" :visible.sync="dialogFormVisible">
      <i-form
        :model="previewModel"
        :formItems="previewFormItems"
        :formAttrs="widgetConfig.formConfig"
      ></i-form>
    ...
    <el-dialog title="表单数据" :visible.sync="dialogFormDataVisible">
      <pre style="overflow-y: auto">{{
        JSON.stringify(previewModel, null, 4)
      }}</pre>
      <div slot="footer" class="dialog-footer">
    ...
    <el-dialog title="JSON预览" :visible.sync="dialogJSONVisible">
      <pre style="height: 400px; overflow-y: auto">{{
        JSON.stringify(widgetConfig, null, 4)
      }}</pre>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogJSONVisible = false">取 消</el-button>
        <el-button type="primary" @click="dialogJSONVisible = false"
          >确 定</el-button
        >
      </div>
    </el-dialog>
    <!-- dialog code -->
    <el-dialog title="代码预览" :visible.sync="dialogCodeVisible">
      <pre style="height: 400px; overflow-y: auto">{{ componentCode }}</pre>
      <div slot="footer" class="dialog-footer">
...
export default {
      ...
      get() {
        ...
        if (this.widgetConfig.formItems.length > 0) {
          this.widgetConfig.formItems.forEach(item => {
            if (
              item.renderConfig &&
              item.renderConfig.defaultValue !== undefined
            ) {
              model[item.prop || item.key] =
                item.renderConfig && item.renderConfig.defaultValue;
            } else {
              model[item.prop || item.key] = "";
            }
          });
  ...
  methods: {
    ...
    viewCode() {
      this.componentCode = GenerateCode({
        model: JSON.stringify(this.modelValue),
        formItems: JSON.stringify(this.widgetConfig.formItems),
        formAttrs: JSON.stringify(this.widgetConfig.formConfig)
      });
