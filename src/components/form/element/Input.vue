<template>
  ...
  >
    <el-button
      v-if="renderConfig.suffix"
      slot="suffix"
      type="primary"
      size="small"
      @click="renderConfig.suffix.click"
    >
      {{ renderConfig.suffix.text }}
    </el-button>
    <template v-if="renderConfig.prepend" slot="prepend">{{
      renderConfig.prepend
    }}</template>
    <template v-if="renderConfig.append" slot="prepend">{{
      renderConfig.append
    }}</template>
  </el-input>
</template>
...
export default {
  ...
  data() {
    return {
      val: this.value,
      autocomplete:
        this.renderConfig.subtype === "password" ? "new-password" : undefined
    };
  },
  methods: {
    input(val) {
